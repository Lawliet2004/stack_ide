...
> - **Phase 4/5:** git per-hunk stage/unstage + commit amend and the DAP debugger bottom-panel UX are implemented and CI green. Remaining are policy-gated/roadmap slices (didSave policy ask, snippet tab-stops, extra grammars, signed marketplace, notebooks/REPL, devcontainers, real SSH/SFTP) plus the final clippy/format cleanup. `didSave` is deliberately unchanged pending approval. CI gates on `cargo check` + `cargo test --lib` + `cargo test --test integration_test`; `cargo clippy --lib -- -D warnings` and `cargo fmt --check` run non-blocking.
