...
> - **Phase 4/5:** remaining feature slices (git per-hunk, didSave policy ask, DAP UX, full SSH, marketplace, notebooks, devcontainers, extra grammars, snippet tab-stops) and doc/CI cleanup are still open. `didSave` is deliberately unchanged pending approval. Docs/README/parity updated. CI now gates on `cargo check` + `cargo test --lib` + `cargo test --test integration_test`; `cargo fmt --check` runs non-blocking and `cargo clippy --lib -- -D warnings` remains non-blocking until the tree's ~50 pre-existing warnings are resolved.
